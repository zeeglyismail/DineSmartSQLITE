...
class UsedIngredientViewSet(viewsets.ModelViewSet):
    ...
    def calculate_amount(self, request, *args, **kwargs):
        # If the 'ingredients' key is not present in the request payload, return success with total 0
        if 'ingredients' not in request.data:
            return Response({"success": True, "total_amount": 0.0}, status=status.HTTP_200_OK)
