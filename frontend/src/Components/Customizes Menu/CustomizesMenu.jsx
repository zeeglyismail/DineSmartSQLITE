...
const CustomizesMenu = () => {
  ...
  // disable actions when total amount is zero
  const isActionDisabled = totalAmout <= 0;
...
const handleSavecombination = () => {
  ...
  if (totalAmout <= 0) {
    Swal.fire({ title: "Please select ingredients", icon: "warning" });
    return;
  }
...
let Table_id = getWithExpiry('table_id') || [];
...
const handleAddtoCart = () => {
  ...
  addToCart = { "order_type" : "dine_in" , "table" : Table_id  , "menu_item" : id , ingredients: quantityData.ingredients}
  }

  else {
   addToCart = { "order_type" : "dine_in" , "table" : Table_id , "saved_meals" : saved_meals , "menu_item" : id , ingredients: quantityData.ingredients}
  }
...
console.log(addToCart);
  ...
  // console.log(combinationdata);
   ...
   fetch(`${base_api}/api/cart/`,{
    ...
    <div className="shadow-[0_0_15px_rgba(0,0,0,0.2)] pt-[10px] w-[98%] lg:w-[100%] rounded-[5px]">
               ...
               <p className="border-b-[2px] w-full"></p>
          ...
          {/* The information of the item show here */}
            ...
            : <div>
              <button
                ...
                title={isActionDisabled ? 'Please select ingredients' : ''}
                className={`bgorg flex items-center gap-[5px] text-white font-semibold py-[5px] w-[350px] md:w-[320px] lg:w-[450px] justify-center mt-[10px] rounded-[5px] ${isActionDisabled ? 'opacity-60 cursor-not-allowed' : ''}`}
              ><FaCartPlus></FaCartPlus>Add to Cart</button>

              <button
                onClick={() => { if (!isActionDisabled) document.getElementById('my_modal_1').showModal(); }}
                disabled={isActionDisabled}
                title={isActionDisabled ? 'Please select ingredients' : ''}
                className={`flex items-center gap-[5px] border-2 border-[#FF6440] org font-semibold py-[5px] w-[350px] md:w-[320px] lg:w-[450px] justify-center mt-[10px] rounded-[5px] ${isActionDisabled ? 'opacity-60 cursor-not-allowed' : ''}`}
              ><FaBookmark></FaBookmark>Save This Combination </button>
            </div>
  ...
  <dialog id="my_modal_1" className="modal">
    <div className="modal-box dark:bg-white">
     <h3 className="font-bold text-lg text-center">Enter  the Combination Name</h3>
           <input onChange={(e) => setCombinationName(e.target.value)} value={combinationName} className=" border-2 border-[#FF6440] mt-[10px] w-[100%] mx-auto rounded-[5px] py-[3px] px-[10px] outline-none dark:bg-white" type="text" />
