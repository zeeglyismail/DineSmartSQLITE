...
const Table = () => {
  ...
  const lo = useLocation();
    ...
    const [tableData , setTableData] = useState([]);
        ...
        // use storageWithExpiry helper: table selection should expire after 1 minute
        let ids = getWithExpiry('table_id') || [];
  ...
  // Load current values (respect expiry)
  let ids = getWithExpiry('table_id') || [];
  let numbers = getWithExpiry('table_number') || [];
  let seats = getWithExpiry('available_seats') || [];
  ...
  // Save updated values with 1 minute TTL
  try {
    setWithExpiry('table_id', ids, 60 * 1000);
    setWithExpiry('table_number', numbers, 60 * 1000);
    setWithExpiry('available_seats', seats, 60 * 1000);
  } catch (err) {
